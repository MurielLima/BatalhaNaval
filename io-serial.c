...
void lerTeclado(int fd,int kbd,char let){
  switch(let){
    case '\n': write(fd,&buf,BUF);limpaBuffer();break;/* Send it to the console "kbd"*/
    case 8 : removeBuffer(kbd); break;
    default: insereBuffer(let,kbd);break;
  }
...
/* Wait for any activitiy in the serial port (fd) or keyboard (kbd) */
  ...
  while(read(fd, &vet, BUF) >= 0){       /* Get a char from serial port "fd"*/
        write(kbd, &vet, BUF);          /* Send it to the console "kbd"*/
  }
  while(read(kbd,&letra,1) >= 0){       /* Get a char from "kbd" (keyboard)read(kbd,&letra,1)*/
